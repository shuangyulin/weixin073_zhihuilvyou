...
export default {
  ...
  methods: {
    ...
    jingdianmingchengChange () {
      this.$http({
        url: `follow/lvyoujingdian/jingdianmingcheng?columnValue=`+ this.ruleForm.jingdianmingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.fenlei){
	...
	this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../ssm49085/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jingquhuodong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingquhuodongCrossAddOrUpdateFlag = false;
                  this.parent.search();
